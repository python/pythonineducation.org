...
(function (TDev) {
    ...
    function receive(message) {
        ...
        switch (message.type) {
            case 0 /* Initialise the editor */:
                setupEditor(message);
                setupButtons(message.translate);
    ...
    /*
    Given an object specifying the language lookup table, update the UI to the
    correct textual values. Used to make the editor labelled in different
    human languages. The translate object MUST look something like the
    following schema:

    {
        ...
        'code_snippets': {
            ...
            'instructions': "Select one of the snippets below, or type the snipp
            et's trigger then tap the TAB key.",
            ...
            'with': 'do some stuff with somethings assigned to a name',
            'cl': 'create a new class that defines the behaviour of a new type o
            f object',
            'def': 'define a named function that takes some arguments and option
            ally add a description',
            'if': 'if some condition is True, do something',
            'ei': 'else if some other condition is True, do something',
            'el': 'else do some other thing',
            'for': 'for each item in a collection of items do something with eac
            h item',
            'try': 'try doing something and handle exceptions (errors)'
        }
    }
    */
    function setupTranslation(translate) {
        try {
            $('#command-quit').attr('title', translate['my_scripts-title']);
            $('#command-quit .roundlabel').text(translate['my_scripts-label']);
            $('#command-download').attr('title', translate['download-title']);
            $('#command-download .roundlabel').text(translate['download-label']);
            $('#command-snippet').attr('title', translate['snippets-title']);
            $('#command-snippet .roundlabel').text(translate['snippets-label']);
            $('#command-help').attr('title', translate['help-title']);
            $('#command-help .roundlabel').text(translate['help-label']);
            $('#zoom-in i').attr('title', translate['zoom_in-title']);
            $('#zoom-out i').attr('title', translate['zoom_out-title']);
            $('#link-log i').attr('title', translate['log-title']);
        } catch (e) {
            console.log(e);
        }
    }
    ...
    function setupEditor(message) {
        // Setup the UI to the expected language / locale
        if(message.translate) {
            setupTranslation(message.translate);
        }
    ...
    function doSnippets(translate) {
        ...
        var context = {
            'title': translate.code_snippets.title,
            'description': translate.code_snippets.description,
            'instructions': translate.code_snippets.instructions,
            'trigger_heading': translate.code_snippets.trigger_heading,
            'description_heading': translate.code_snippets.description_heading,
            'snippets': snippetManager.snippetMap.python,
            'describe': function() {
                return function(text, render) {
                    name = render(text);
                    trigger = name.split(' - ')[0];
                    return translate.code_snippets[trigger];
                }
            }
        }
        vex.open({
    ...
    function setupButtons(translate) {
